...
const char filename[] = "tests/test1.txt"; //Корректный тест
...
void Print_error(int type, char* break_str) {
    ...
    if (type == 5) cout << "Ошибка! Нехватает данных для";
    if (type == 7) cout << "Ошибка! Время посадки отсутствует";
    if (type == 8) cout << "Ошибка! Некорректный номер рейса";
