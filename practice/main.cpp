...
int Read_data(const char* file_name, Aircraft *&all_arr,
...
void Print_message(int code, char* brand);
...
int main() { // Начало main
    Aircraft* all_arr = nullptr; // Массив всех ЛА
    ...
    // Читаем файл
    int err = Read_data(filename, all_arr, total_lines, valid_arr, valid_count, B);
    if (err == 1|| err == 2)
    ...
    int &total_lines, Aircraft *&valid_arr, int &valid_count, int*&B)
{
    ...
    if (!file_0.is_open()) return 1;
    if (file_0.peek() == EOF) return 2;
    ...
    if (total_lines == 0) return 3; // Строк нет - ошибка (файл пуст)
...
void Copy_All(Aircraft& dest, const Aircraft& src, bool valid) {
    ...
    dest.boarding_hour = src.boarding_hour;
    dest.boarding_min = src.boarding_min;
    dest.is_valid = valid;
}
...
int Check_duplicates(Aircraft *arr, int valid_count, Aircraft &new_aircraft, char* break_str) {
    ...
    {// Начало цикла обработки строк
        ...
        if (strcmp(arr[k].tail_number, new_aircraft.tail_number) == 0 && strcmp(arr[k].brand, new_aircraft.brand) == 0)
        {
            ...
            {
                // Одинаковый борт, одиноковое время посадки
        ...
        {
            // Одинаковый номер рейса, одиноковое время посадки
            if (arr[k].boarding_hour == new_aircraft.boarding_hour && arr[k].boarding_min == new_aircraft.boarding_min)
            {
                // Одинаковый борт, одиноковое время посадки
                strcpy(break_str, new_aircraft.brand);
...
int Check_tail_number(char* num) {
    ...
    if (num == nullptr || num[0] == '\0') return 4;
    int i = 0; // Индекс поля
    ...
    if (num[i+len_tail] != '\0') return 4; // Проверка длины цифровой части
    return 0;
}
...
int Check_time(char* extra_s, unsigned int &hour, unsigned int &min){
    ...
    if (hour > 23 || min > 59) return 6; // Вывод за границы формата времени
